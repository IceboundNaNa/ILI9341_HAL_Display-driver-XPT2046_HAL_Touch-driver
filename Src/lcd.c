 ...
 * @retval      无
 */
void lcd_show_string(uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint8_t size, uint8_t mode, char *p, uint16_t color)
{
    uint8_t x0 = x;
    ...
    while ((*p <= '~') && (*p >= ' '))
    {
        ...
        lcd_show_char(x, y, *p, size, mode, color); // <--- 将传入的 mode 参数传递下去
        x += size / 2;
        p++;
    }
}
...
/**
 * @brief       在指定座标范围显示一副图片
 * @param       StartX, StartY: 图片左上角起始坐标
 * @param       EndX, EndY:     图片右下角结束坐标
 * @param       pic:            图片数据数组(16-bit color, RGB565 format)
 * @retval      无
 * @note        图片取模格式为水平扫描，16位颜色模式
 */
void lcd_draw_picture(uint16_t StartX, uint16_t StartY, uint16_t EndX, uint16_t EndY, const unsigned char *pic)
{
    /* 1. Calculate width, height, and total number of pixels */
    uint16_t width = EndX - StartX ;
    uint16_t height = EndY - StartY ;
    uint32_t total_pixels = (uint32_t)width * height;
    uint32_t i;

    /* 2. Create a correctly typed pointer to the 16-bit color data */
    const uint16_t *p_color = (const uint16_t *)pic;

    /* 3. Set the drawing window with the correct parameters (start coordinates, width, height) */
    lcd_set_window(StartX, StartY, width, height);

    /* 4. Prepare the LCD to receive pixel data (this only needs to be done once) */
    lcd_write_ram_prepare();

    /* 5. Loop through all pixels and write them to the LCD's GRAM efficiently */
    for (i = 0; i < total_pixels; i++)
    {
        /* Directly write the 16-bit color data. lcd_wr_data() is a good wrapper for this. */
        lcd_wr_data(p_color[i]);
    }
}
