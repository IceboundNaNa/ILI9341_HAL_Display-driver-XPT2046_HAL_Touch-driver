...
void lcd_show_string(uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint8_t size, uint8_t mode, char *p, uint16_t color);/* 显示字符串 */
void lcd_draw_picture(uint16_t StartX, uint16_t StartY, uint16_t EndX, uint16_t EndY, const unsigned char *pic);         /* 画图片 */
